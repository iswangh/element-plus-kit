# @iswangh/element-plus-kit

## 0.3.5

### Patch Changes

- 新增 dialog drawer 包，统一在各包中的 index.ts 中导出对外使用的数据
- Updated dependencies
  - @iswangh/element-plus-kit-dialog@0.0.2
  - @iswangh/element-plus-kit-drawer@0.0.2
  - @iswangh/element-plus-kit-form@0.3.3
  - @iswangh/element-plus-kit-tag@0.0.3

## 0.3.4

### Patch Changes

- form 包导入 WCheckTag 组件样式
- Updated dependencies
  - @iswangh/element-plus-kit-form@0.3.2

## 0.3.3

### Patch Changes

- 更新依赖、form 集成 WCheckTag
- Updated dependencies
  - @iswangh/element-plus-kit-form@0.3.1
  - @iswangh/element-plus-kit-tag@0.0.2

## 0.3.2

### Patch Changes

- 修复 WCheckTag 无法自动导入的问题

## 0.3.1

### Patch Changes

- 新增 tag checkTag 组件
- Updated dependencies
  - @iswangh/element-plus-kit-tag@0.0.1

## 0.3.0

### Minor Changes

- 重构 optionsLoader ,修改为用户使用 useLoadOptions 自主调用 loader

### Patch Changes

- Updated dependencies
  - @iswangh/element-plus-kit-form@0.3.0

## 0.2.1

### Patch Changes

- 可配置化插槽事件
- Updated dependencies
  - @iswangh/element-plus-kit-form@0.2.1

## 0.2.0

### Minor Changes

- 现有功能实现重构

### Patch Changes

- Updated dependencies
  - @iswangh/element-plus-kit-form@0.2.0

## 0.1.4

### Patch Changes

- 优化 Form 组件文档和实现，修复 placeholder 默认值描述，完善 API 文档说明
- Updated dependencies
  - @iswangh/element-plus-kit-form@0.1.4

## 0.1.3

### Patch Changes

- 95e2f17: chore: 使用 changeset 优化发布流程
- Updated dependencies [95e2f17]
  - @iswangh/element-plus-kit-form@0.1.3
