...
export default defineConfig({
  ...
  build: {
    ...
    rollupOptions: {
      ...
      //   - @iswangh/element-plus-kit-form：在 dependencies 中声明，但标记为外部依赖，运行时通过包管理器解析
      //   - @iswangh/element-plus-kit-core：虽然 kit 包不直接依赖 core 包，但 form 包依赖了 core 包，为了确保 core 包不会被意外打包，也将其标记为外部依赖
      external: [
        'vue',
        'element-plus',
