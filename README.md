...
```bash
# 发布所有包（使用 Changesets，按依赖顺序发布）
pnpm release
```

**发布流程**：
1. 按依赖顺序发布包（core → form → kit）
2. 每个包的 `prepublishOnly` 钩子会自动执行构建和类型检查
3. 构建和类型检查通过后，自动发布到 npm

**注意**：分支检查已集成到 Changesets 的 pre 钩子中（使用 `@iswangh/script` 包），执行 `pnpm version` 或 `pnpm release` 时会自动检查当前分支是否为 `main` 或 `master`。
