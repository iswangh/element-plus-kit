...
```bash
# 发布所有包（使用 Changesets，自动检查分支并按依赖顺序发布）
pnpm release
```

**发布流程**：
1. 自动检查当前分支是否为 `main` 或 `master`（通过 Changesets pre 钩子）
2. 如果分支检查失败，发布会被阻止
3. 如果分支检查通过，按依赖顺序发布包（core → form → kit）
4. 每个包的 `prepublishOnly` 钩子会自动执行构建和类型检查
5. 构建和类型检查通过后，自动发布到 npm
...
# 发布包（会自动检查分支，通过 Changesets pre 钩子）
pnpm release
```
