# Changesets 使用指南

## 📖 概述

Changesets 是专为 Monorepo 设计的版本管理和发布工具，用于：
- 自动管理版本号（遵循语义化版本 SemVer）
- 自动处理依赖顺序和版本更新
- 自动生成变更日志（CHANGELOG.md）
- 简化发布流程

## 🚀 快速开始

### 1. 创建变更集（Changeset）

在代码修改后，创建变更集文件来记录变更：

```bash
pnpm changeset
```

或者使用：

```bash
nr changeset
```

交互式选择：
- **受影响的包**：选择 core、form、kit 中的一个或多个
- **变更类型**：
  - `patch`：Bug 修复、性能优化（0.1.2 → 0.1.3）
  - `minor`：新功能、向下兼容的增强（0.1.2 → 0.2.0）
  - `major`：破坏性变更、不兼容的 API 修改（0.1.2 → 1.0.0）
- **变更描述**：简要描述本次变更

### 2. 提交变更集文件

创建变更集后，提交到版本控制：

```bash
git add .changeset/
git commit -m "chore: add changeset"
git push
```

### 3. 更新版本号

合并到 main 分支后，根据变更集更新版本号：

```bash
pnpm version
```

或者：

```bash
nr version
```

这个命令会：
- 自动检查分支（通过 Changesets pre 钩子，必须在 main 或 master 分支）
- 读取 `.changeset/` 目录下的变更集文件
- 自动更新包的版本号（根据变更类型）
- 自动更新内部依赖版本（如果被依赖包版本变化）
- 生成或更新 CHANGELOG.md
- 删除已处理的变更集文件

**注意**：分支检查已集成到 Changesets 的 pre 钩子中，无需手动检查。

### 4. 提交版本更新

版本更新后，提交更改：

```bash
git add .
git commit -m "chore: version packages"
git push
```

### 5. 发布包

发布所有需要发布的包：

```bash
pnpm release
```

或者：

```bash
nr release
```

这个命令会：
- 自动检查分支（通过 Changesets pre 钩子，必须在 main 或 master 分支）
- 按依赖顺序发布到 npm（core → form → kit）
- 每个包的 `prepublishOnly` 钩子会自动执行构建和类型检查
- 自动处理内部依赖版本更新

**注意**：
- 分支检查已集成到 Changesets 的 pre 钩子中，无需手动检查
- 构建由每个包的 `prepublishOnly` 钩子自动执行，无需在 `release` 命令中手动构建

## 📝 完整发布流程示例

```bash
# 1. 开发完成后，创建变更集
pnpm changeset
# 交互式选择：选择包、变更类型、输入描述

# 2. 提交变更集文件
git add .changeset/
git commit -m "chore: add changeset"
git push

# 3. 合并到 main 分支后，更新版本号
pnpm version
# 这会更新版本号、依赖版本和 CHANGELOG

# 4. 提交版本更新
git add .
git commit -m "chore: version packages"
git push

# 5. 发布包
pnpm release
# 这会发布所有需要发布的包到 npm
```

## ⚙️ 配置说明

配置文件：`.changeset/config.json`

```json
{
  "access": "public",                    // 包发布为公开包
  "baseBranch": "main",                  // 基础分支
  "updateInternalDependencies": "patch", // 内部依赖更新策略
  "commit": false                        // 不自动提交（手动控制）
}
```

## ✨ Changesets 的优势

1. **自动化依赖顺序**：自动按依赖顺序发布（core → form → kit）
2. **自动版本更新**：根据变更类型自动更新版本号
3. **自动依赖更新**：自动更新内部依赖版本
4. **自动变更日志**：自动生成 CHANGELOG.md
5. **变更审查**：变更集文件可以提交 PR 审查
6. **避免错误**：不会因为工作区不干净而失败

## 📚 更多资源

- [Changesets 官方文档](https://github.com/changesets/changesets)
- [常见问题](https://github.com/changesets/changesets/blob/main/docs/common-questions.md)
- [项目规范](../.cursor/rules/001_monorepo_development-standards.mdc)
